...
namespace RentAVilla_Web.Services
{
    public class BaseService : IBaseService
    {
        ...
        public async Task<T> SendAsync<T>(APIRequest apiRequest)
        {
            try
            {
                var client = httpClient.CreateClient("RentAVillaAPI");
                HttpRequestMessage message = new();
                message.Headers.Add("Accept", "application/json");
                message.RequestUri = new Uri(apiRequest.Url);
				if (apiRequest != null && (apiRequest.ApiType == SD.ApiType.POST || apiRequest.ApiType == SD.ApiType.PUT))
				{
                ...
                {
					var exceptionRequest = JsonConvert.DeserializeObject<T>(apiContent);
